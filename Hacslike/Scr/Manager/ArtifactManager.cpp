...
void ArtifactManager::SaveToArtifactData(ArtifactData& outData)
{
    outData.obtained.clear();
    outData.notobtained.clear();

    // --- 獲得済み ---
    for (auto& a : obtainedArtifacts)
    {
        outData.obtained.push_back(a->GetID());
    }

    // --- 未取得（プール）---
    for (auto& a : artifacrPool)
    {
        outData.notobtained.push_back(a->GetID());
    }
}

void ArtifactManager::LoadFromArtifactData(const ArtifactData& data)
{
    obtainedArtifacts.clear();
    artifacrPool.clear();
    activeArtifact.clear();

    // --- まず "全 ArtifactBase" を構築 ---
    // これは Start() などにある artifactPool 初期化を関数化すると良い
    artifacrPool = {
        std::make_shared<conditional_attack_power_raise_HP>(),
        std::make_shared<Hp_Max_Raise>(),
        std::make_shared<CoinValue_raise>(),
    ...
    // --- プールを復元 ---
    std::vector<std::shared_ptr<ArtifactBase>> newPool;
    for (auto& a : artifacrPool)
    {
        if (std::find(data.notobtained.begin(), data.notobtained.end(), a->GetID()) != data.notobtained.end())
        {
            newPool.push_back(a);
        }
    }
    artifacrPool = newPool;

    // --- 獲得済みの復元 ---
    for (int id : data.obtained)
    {
        for (auto& a : artifacrPool)
        {
            if (a->GetID() == id)
            {
                obtainedArtifacts.push_back(a);
                break;
            }
        }
    }
