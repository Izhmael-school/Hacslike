...
// コンストラクタ
DamagePopup::DamagePopup(float x, float y, float z, int damage, bool isCritical)
    : x(x), y(y), z(z), damage(damage), isCritical(isCritical), timer(60), alpha(255) {
}
...
void DamagePopup::Create(Character* ch, int damage, bool isCritical) {
    ...
    instances.push_back(new DamagePopup(pos.x, pos.y + headOffset, pos.z, damage, isCritical));
}
...
void DamagePopup::RenderAll() {
    for (auto p : instances) {
        ...
        if (screenPos.z >= 0.0f && screenPos.z <= 1.0f) {
            ...
            unsigned int color;
            if (p->isCritical) {
                color = GetColor(255, 255, 0); // クリティカル：黄色
            }
            else {
                color = GetColor(255, 255, 255); // 通常：白
            }
