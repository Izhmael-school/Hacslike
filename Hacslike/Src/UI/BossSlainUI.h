...
public:
    ...
    inline void Draw() {
        ...
        // 2. 背景帯（画面中央に高さを合わせて描画）
        // 以前の 240〜360（高さ120）を維持する場合
        int rectHalfHeight = 60;
        DrawBox(0, centerY - rectHalfHeight, WINDOW_WIDTH, centerY + rectHalfHeight, GetColor(0, 0, 0), TRUE);

        // 3. テキスト表示
        // DrawExtendStringの座標は「左上」なので、文字列の幅を考慮して中心に寄せます
        // ※文字サイズ5倍、フォントデフォルトの場合の概算
        if (StageManager::GetInstance().floorCount > 50) {
            // "GOD SLAIN" は 9文字
        ...
        // 4. 花火の描画
        // (fireworksのx, yが生成時にWINDOW_WIDTH等を使って計算されていれば、ここは変更不要です)
